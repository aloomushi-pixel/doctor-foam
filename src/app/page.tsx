    ...
    <>
      ...
      <section className="section-padding" id="proceso">
        <div className="container" style={{ textAlign: "center" }}>
          <div className="animate-on-scroll">
            ...
            <h2 className="section-title">
              4 pasos hacia la <span className="gradient-text">perfección</span>
            </h2>
          </div>

          <div className="process-grid" style={{ maxWidth: "1000px", margin: "0 auto" }}>
            {processSteps.map((s, i) => (
              <div key={i} className="glass-card process-step animate-on-scroll">
                <div className="process-number">{s.num}</div>
                <h3 style={{ fontSize: "1.1rem", marginBottom: "0.5rem" }}>{s.title}</h3>
                <p style={{ color: "#475569", fontSize: "0.9rem" }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding" id="resenas" style={{ background: "#f1f5f9" }}>
        <div className="container" style={{ textAlign: "center" }}>
          ...
          {/* Reviews Grid */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.25rem", maxWidth: "1100px", margin: "2.5rem auto 0" }}>
            {googleReviews.map((r, i) => (
              <div key={i} className="glass-card animate-on-scroll" style={{ padding: "1.5rem", textAlign: "left", position: "relative" }}>
                {/* Review Header */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    {/* Avatar */}
                    <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "linear-gradient(135deg, #4285F4, #34A853)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: "1rem", flexShrink: 0 }}>
                      {r.author.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p style={{ fontWeight: 600, fontSize: "0.9rem", color: "#0f172a", margin: 0 }}>{r.author}</p>
                      <p style={{ color: "#94a3b8", fontSize: "0.75rem", margin: 0 }}>{r.date}</p>
                    </div>
                  </div>
                  {/* Google G icon */}
                  <svg width="18" height="18" viewBox="0 0 24 24" style={{ flexShrink: 0, opacity: 0.6 }}>
                ...
                {/* Stars */}
                <div style={{ color: "#FBBC05", fontSize: "0.9rem", letterSpacing: "1px", marginBottom: "0.75rem" }}>
                  {"★".repeat(r.stars)}
                </div>
                {/* Review Text */}
                <p style={{ color: "#334155", fontSize: "0.88rem", lineHeight: "1.7", margin: 0 }}>
                  &ldquo;{r.text}&rdquo;
                </p>
              </div>
            ))}

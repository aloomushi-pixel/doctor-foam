  ...
  try {
    ...
    <div style="padding:2rem;">
      ...
      <div style="text-align:center;margin:1.5rem 0;">
        <a href="${WHATSAPP_URL}?text=${encodeURIComponent(`Hola, soy ${firstName}. Acabo de confirmar mi servicio ${packageName} para el ${serviceDate}. ¿Podrían confirmarme el horario?`)}" style="display:inline-block;background:linear-gradient(135deg,#25d366,#128c7e);color:white;text-decoration:none;padding:0.75rem 2rem;border-radius:8px;font-weight:600;font-size:0.9rem;">📱 Escríbenos por WhatsApp</a>
      </div>
    </div>${emailFooter()}`,
    });
  ...
  try {
    ...
    <div style="padding:1.5rem 2rem;">
      ...
      <div style="text-align:center;margin-top:1.25rem;">
        <a href="${WHATSAPP_URL}?text=${encodeURIComponent(`Cliente: ${customerName} - ${customerPhone}`)}" style="display:inline-block;background:linear-gradient(135deg,#25d366,#128c7e);color:white;text-decoration:none;padding:0.6rem 1.5rem;border-radius:8px;font-weight:600;font-size:0.85rem;">📱 Contactar por WhatsApp</a>
      </div>
    </div>${emailFooter()}`,
    });
...
/* ═══════════════════════════════════════════════
   3. RECORDATORIO PRE-SERVICIO (24h antes)
   ═══════════════════════════════════════════════ */
  ...
  try {
    ...
    <div style="padding:2rem;">
      ...
      <div style="background:rgba(251,191,36,0.08);border-left:3px solid #fbbf24;border-radius:0 8px 8px 0;padding:1rem;">
        ...
        <p style="color:#cbd5e1;font-size:0.85rem;margin:0;line-height:1.6;">Asegúrate de que el vehículo esté accesible. Si necesitas hacer cambios, contáctanos por WhatsApp.</p>
      </div>

      <div style="text-align:center;margin-top:1.5rem;">
        <a href="${WHATSAPP_URL}?text=${encodeURIComponent(`Hola, soy ${firstName}. Tengo mi servicio agendado para mañana y quiero confirmar.`)}" style="display:inline-block;background:linear-gradient(135deg,#25d366,#128c7e);color:white;text-decoration:none;padding:0.75rem 2rem;border-radius:8px;font-weight:600;font-size:0.9rem;">📱 Contactar por WhatsApp</a>
      </div>
    </div>${emailFooter()}`,
    });
